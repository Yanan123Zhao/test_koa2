...
fs.readFile('../qiniu.js', () => {
  ...
  setImmediate(() => {
    ...
    fs.readFile('../qiniu.js', () =>{
      console.log('[阶段2 IO] 读取文件 3')
      setImmediate(() => {console.log('[阶段3 Immediate] 3 读取文件 immediate 3')})
      setTimeout(() =>{
        console.log('[阶段1 定时器] 4')
      })
    })
  })
  process.nextTick(() => {
    console.log('[待进入下个阶段] 读取文件 nextTick 1')
  })
  setTimeout(() => {
    console.log('[阶段1 定时器] 5')
  }, 0);
})
